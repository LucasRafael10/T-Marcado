...
import { resetEmailHtml } from "../src/email-templates.mjs";

test("e-mail de recuperação inclui botão, link alternativo e escapa a URL", () => {
  const html = resetEmailHtml(
    'https://exemplo.com/redefinir-senha.html#token=abc&next="teste"',
  );
  assert.match(html, /Criar nova senha/);
  assert.match(html, /30 minutos/);
  assert.match(html, /abc&amp;next=&quot;teste&quot;/);
  assert.doesNotMatch(html, /abc&next="teste"/);
});
...
test("Resend: envio, falhas e diagnóstico sem dados sensíveis", async () => {
  ...
  try {
    ...
    await assert.rejects(
      sendEmail("cliente@example.com", "Assunto", "Texto"),
      /Configure/,
    );
    ...
    assert.ok(
      !emailDiagnostic(new Error("token secreto")).includes("token secreto"),
    );
  } finally {
